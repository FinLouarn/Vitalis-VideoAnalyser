...
weaviate_url = "https://btyrqutspoquragfjelca.c0.europe-west3.gcp.weaviate.cloud"
weaviate_api_key = "aUYzT3JrYUNpRUxCeEQ4aF9TRkRHZFU0L0dBYkEwWkJLUzhnbnpYQ0c3TVZXejRpNHNFOGtvNVAvSlpjPV92MjAw"
    ...
    def process_frame(self, save_path: str = None):
        ...
        with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
            ...
            with weaviate.connect_to_weaviate_cloud(
                ...
                collection = client.collections.get("CollectionPhysio")
                enriched_issues = []
                for issue in self.analysis_results["detected_issues"]:
                    issue_type = issue["issue_type"]

                    # Query for best match in Weaviate
                    result = collection.query.fetch_objects(
                        filters=Filter.by_property("issue_type").equal(issue_type),
                        limit=1  # get top match
                    )

                    if result.objects:
                        best_match = result.objects[0].properties
                        # Merge original issue with retrieved recommendation
                        enriched_issue = {
                            **issue,
                            "description": best_match.get("description"),
                            "severity_guidelines": best_match.get("severity_guidelines"),
                ...
                # Final enriched report
                enriched_report = {
                    "exercise": self.analysis_results["exercise"],
                    "total_squat": self.analysis_results["total_squat"],
                    "total_perfect_depth_squat": self.analysis_results["total_perfect_depth_squat"],
                    "issues_report": enriched_issues
                }

            with open("squat_analysis_results_enriched_issues.json", "w") as f:
